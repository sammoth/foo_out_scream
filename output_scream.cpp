...
static const GUID guid_scream_branch = {
    0x18564ced,
    0x4abf,
    0x4f0c,
    {0xa4, 0x43, 0x98, 0xda, 0x88, 0xe2, 0xcd, 0x39}};
static const GUID guid_cfg_destination_port = {
    0xe7598963,
    0xed60,
    0x4084,
    {0xa8, 0x5d, 0xd1, 0xcd, 0xc5, 0x51, 0x22, 0xca}};

static advconfig_branch_factory g_scream_output_branch(
    "Scream output", guid_scream_branch, advconfig_branch::guid_branch_playback,
    0);
static advconfig_integer_factory cfg_scream_destination_port(
    "Destination port", guid_cfg_destination_port, guid_scream_branch, 0, 4010,
    0, 65535, 0);
...
class scream_player {
 public:
  ...
  scream_player(double buffer_length_seconds, t_samplespec p_spec,
                size_t p_bitdepth, bool p_dither, int port)
      : max_size_(buffer_length_seconds * p_bitdepth * p_spec.m_channels *
                  p_spec.m_sample_rate / 8),
        buf_(std::unique_ptr<BYTE>(
            new BYTE[buffer_length_seconds * p_bitdepth * p_spec.m_channels *
                     p_spec.m_sample_rate / 8])),
        bitdepth(p_bitdepth),
        dither(p_dither),
        spec(p_spec),
        ...
        m_bSamplingFreqMarker(
            (BYTE)((spec.m_sample_rate % 44100)
                       ? (0 + (spec.m_sample_rate / 48000))
                       : (128 + (spec.m_sample_rate / 44100)))),
        waiting(true),
        send_ahead_frames(p_spec.time_to_samples(0.001 * SEND_AHEAD_MS) *
                          p_spec.m_channels * p_bitdepth / 9216) {
    switch (spec.m_channel_config) {
      case audio_chunk::channel_config_mono:
        m_wChannelMask = (WORD)KSAUDIO_SPEAKER_MONO;
        break;
      case audio_chunk::channel_config_stereo:
        m_wChannelMask = (WORD)KSAUDIO_SPEAKER_STEREO;
        break;
      case audio_chunk::channel_config_4point0:
        m_wChannelMask = (WORD)KSAUDIO_SPEAKER_QUAD;
        break;
      case audio_chunk::channel_config_5point0:
        m_wChannelMask = (WORD)KSAUDIO_SPEAKER_5POINT0;
        break;
      case audio_chunk::channel_config_5point1:
        m_wChannelMask = (WORD)KSAUDIO_SPEAKER_5POINT1;
        break;
      case audio_chunk::channel_config_7point1:
        m_wChannelMask = (WORD)KSAUDIO_SPEAKER_7POINT1;
        break;
      case audio_chunk::channel_config_5point1_side:
        m_wChannelMask = (WORD)KSAUDIO_SPEAKER_5POINT1;
        break;
      default:
        m_wChannelMask = (WORD)KSAUDIO_SPEAKER_STEREO;
    ...
    timer_resolution = max(min(max(tc.wPeriodMin, 0), tc.wPeriodMax),
                           TIMER_TARGET_RESOLUTION_MS);
    timeBeginPeriod(timer_resolution);
    timer_id =
        timeSetEvent(TIMER_INTERVAL_MS, timer_resolution, TimerCallback,
                     (DWORD)this, TIME_PERIODIC || TIME_KILL_SYNCHRONOUS);
  ...
  void force_play() { force_play_flag = true; }
  bool is_progressing() { return !waiting; }
  void queue(audio_chunk_impl chunk) {
    postprocessor->run(chunk, incoming, bitdepth, bitdepth, dither, 1.0f);
    {
      ...
      memcpy(buf_.get() + head_, incoming.get_ptr(),
             pfc::min_t(copy_amount, endspace));
      if (endspace < copy_amount) {
        memcpy(buf_.get(), ((BYTE*)incoming.get_ptr()) + endspace,
               incoming.get_size() - endspace);
      }
    ...
    if (waiting &&
        size() > pfc::min_t(max_size_ / 2,
                            bitdepth *
                                spec.time_to_samples(BUFFER_BEFORE_PLAYBACK_MS *
                                                     0.001) *
                                spec.m_channels / 8)) {
      waiting = false;
    }
  ...
  size_t can_write_samples() { return space() * 8 / bitdepth; }

  size_t samples_queued() { return size() * 8 / bitdepth; }
 ...
 private:
  ...
  size_t space() const { return max_size_ - size(); }

  static void CALLBACK TimerCallback(UINT wTimerID, UINT msg, DWORD dwUser,
                                     DWORD dw1, DWORD dw2) {
  ...
  void send_frames() {
    ...
    size_t frames_target =
        ceil((double)spec.time_to_samples(elapsed_seconds.count()) *
             spec.m_channels * bitdepth / 9216.0) +
        send_ahead_frames - frames_sent;
    ...
    for (int i = 0; i < frames_target; i++) {
      ...
      if (playing && (force_play_flag || !waiting)) {
        ...
        if (take == size()) force_play_flag = false;
  ...
  void send_udp(BYTE frame[]) {
    char ch = 0;
    int nbytes = sendto(udp_socket_fd, (char*)frame, 1157, 0,
...
class output_scream : public output_impl {
 ...
 public:
  output_scream(const GUID& p_device, double p_buffer_length, bool p_dither,
                t_uint32 p_bitdepth)
      : m_incoming_ptr(0),
        buffer_length_seconds(p_buffer_length),
        bitdepth(p_bitdepth),
        dither(p_dither),
        player(std::unique_ptr<scream_player>(new scream_player())) {}

  void on_update() override {}
  void write(const audio_chunk& p_data) override { player->queue(p_data); }
  ...
  void on_flush() override { player->reset(); }
  void on_flush_changing_track() override { player->reset(); }
  void open(t_samplespec const& p_spec) override {
    if (p_spec.m_sample_rate % 44100 != 0 &&
        p_spec.m_sample_rate % 48000 != 0) {
      pfc::throw_exception_with_message<exception_io_data>(
          "Invalid sample rate for Scream playback - must be multiple of "
          "44100Hz or 48000Hz");
    }

    player = std::unique_ptr<scream_player>(
        new scream_player(buffer_length_seconds, p_spec, bitdepth, dither,
                          cfg_scream_destination_port.get()));
    player->play();
  }
  ...
  void force_play() override { player->force_play(); }
  void volume_set(double p_val) override {}
  bool is_progressing() override { return player->is_progressing(); }
  static void g_enum_devices(output_device_enum_callback& p_callback) {
    const GUID device = {0x290538b,
                         0x27da,
                         0x4ca7,
                         {0x4a, 0x1b, 0xaf, 0x91, 0x8b, 0xa, 0x15, 0xd0}};
    p_callback.on_device(device, "Multicast", 9);
  }
  static GUID g_get_guid() {
    // This is our GUID. Generate your own one when reusing this code.
    static const GUID guid = {0x410960c,
                              0x14bf,
                              0x3491,
                              {0xf8, 0x19, 0x50, 0x6a, 0x9b, 0x63, 0x48, 0x82}};
    return guid;
  }
